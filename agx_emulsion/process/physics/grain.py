...
from agx_emulsion.process.utils.fast_stats import (
    fast_binomial,
    fast_poisson,
...
def layer_particle_model(
    ...
    if method == "gamma_beta":
        ...
        seeds = gamma_rvs(
            n_particles_per_pixel / (1 - grain_uniformity + 1e-6), size=density.shape
        ) * (1 - grain_uniformity + 1e-6)
        grain = (
            beta_rvs(
                probability_of_development * n_particles_per_pixel,
...
def add_micro_structure(density_cmy_out, micro_structure, pixel_size_um):
    ...
    if grain_micro_structure_sigma > 0.05:
        clumping = fast_lognormal_from_mean_std(
            np.ones_like(density_cmy_out),
            np.ones_like(density_cmy_out) * grain_micro_structure_sigma,
        )
        if grain_micro_structure_blur_pixel > 0.4:
            clumping = scipy.ndimage.gaussian_filter(
                clumping, (grain_micro_structure_blur_pixel, grain_micro_structure_blur_pixel, 0)
            )
        density_cmy_out *= clumping
    return density_cmy_out


def apply_grain_to_density(
    density_cmy,
    pixel_size_um=10,
    ...
    for ch in np.arange(3):
        for sl in np.arange(n_sub_layers):
            density_cmy_out[:, :, ch] += layer_particle_model(
                density_cmy[:, :, ch],
    ...
    if sigma_blur_pixel > 0.4:
        density_cmy_out = scipy.ndimage.gaussian_filter(
            density_cmy_out, (sigma_blur_pixel, sigma_blur_pixel, 0)
        )
...
def apply_grain_to_density_layers(
    ...
    pixel_area_um2 = pixel_size_um**2
    agx_particle_area_um2_layers = (
        agx_particle_area_um2
        * np.array(agx_particle_scale)[None, :]
        * np.array(agx_particle_scale_layers)[:, None]
    )  # layers, rgb
    ...
    if grain_blur > 0:
        density_cmy_out = scipy.ndimage.gaussian_filter(
            density_cmy_out, (grain_blur, grain_blur, 0)
        )
    return density_cmy_out
...
if __name__ == "__main__":
    ...
    g1 = layer_particle_model(
        density, density_max=2, n_particles_per_pixel=10, grain_uniformity=0.99, sigma_blur=0.0
    )
    g2 = layer_particle_model(
        density, density_max=2, n_particles_per_pixel=10, grain_uniformity=0.96, sigma_blur=0.0
    )
    print("g1 ------------------")
    print("Density Test")
