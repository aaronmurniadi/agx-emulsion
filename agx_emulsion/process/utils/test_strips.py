...
class TestStrip:
    ...
    def process(self, plot=True):
        strip, labels = self._compute(
            self.image, self.params, self.title, stack=self.stack, plot=True
        )
        self.labels = labels
        if plot:
    ...
    def _compute(self, image, conditions, title, stack="h", plot=True):
        labels = []
        ...
        if stack == "h":
            ...
            if lines:
                for i in np.arange(n_conditions - 1):
                    i = np.double(i)
                    ax.axvline(
                        (i + 1) * np.double(strip.shape[1]) / n_conditions - 0.5,
                        color="w",
                        linewidth=0.5,
                    )
            ax.set_xlabel(title)
        if stack == "v":
            ...
            if lines:
                for i in np.arange(n_conditions - 1):
                    i = np.double(i)
                    ax.axhline(
                        (i + 1) * np.double(strip.shape[0]) / n_conditions - 0.5,
                        color="w",
                        linewidth=0.5,
                    )
            ax.set_ylabel(title)
