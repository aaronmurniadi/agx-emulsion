...
from agx_emulsion.process.profiles.reconstruct import reconstruct_dye_density
from agx_emulsion.process.profiles.balance import balance_sensitivity, balance_metameric_neutral
from agx_emulsion.process.profiles.correct import align_midscale_neutral_exposures

from agx_emulsion.process.physics.density_curves import (
    fit_density_curve,
    compute_density_curves,
    compute_density_curves_layers,
)
from agx_emulsion.process.utils.io import load_agx_emulsion_data, load_densitometer_data
...
def fit_density_curves(log_exposure, density, model="norm_cdfs", type="negative"):
    fitted_parameters = np.zeros((3, 9))
    for i in np.arange(3):
        fitted_parameters[i] = fit_density_curve(log_exposure, density[:, i], type, model=model)
...
def fit_log_scaled_absortion_coefficients(
    ...
    sensitivity_log_exposures[np.isnan(sensitivity_log_exposures)] = (
        np.nanmax(sensitivity_log_exposures) + 6
    )

    def unmixed_densities_at_a_sensitivity_log_exposure(
        log_absorption_coefficients_rgb, sensitivity_log_exposure_i
    ):
        ...
        for i in np.arange(3):
            ...
            log_exposure_with_new_reference = (
                log_exposure_reference_sensitivity + sensitivity_log_exposure_i
            )
            unmixed_densities[i] = np.interp(
                log_exposure_with_new_reference + log_absorption_coefficients_rgb[i],
                log_exposure,
                density_curves[:, i],
            )
    ...
    def densitometer_densities_at_sensitivity_log_exposures(
        log_absorption_coefficients_rgb, sensitivity_log_exposures
    ):
        ...
        for i in np.arange(3):
            ...
            unmixed_densities = unmixed_densities_at_a_sensitivity_log_exposure(
                log_absorption_coefficients_rgb, sensitivity_log_exposures[i]
            )
    ...
    log_absorption_coefficients_0[np.isnan(log_absorption_coefficients_0)] = (
        np.nanmin(log_absorption_coefficients_0) - 6
    )
    for i in np.arange(sensitivity_log_exposures.shape[0]):  # for every i-th wavelength
        ...
        def residues(log_absorption_coefficients_rgb):
            res = target - densitometer_densities_at_sensitivity_log_exposures(
                log_absorption_coefficients_rgb, sensitivity_log_exposures[i, :]
            )
            return res

        fit = scipy.optimize.least_squares(
            residues, log_absorption_coefficients_0[i, :], method="lm"
        )
...
def find_log_exposure_reference(
    log_exposure, density_curves, density_reference=1.0, decreasing_density=False
):
    ...
    for i in np.arange(3):
        log_exposure_reference += np.interp(
            sign * density_reference, sign * density_curves[sel, i], log_exposure[sel]
        )
...
def unmix_sensitivity(profile, control_plot=False):
    ...
    density_curves_densitometer_minus_dmin = np.einsum(
        "ij,kj->ki", densitometer_crosstalk_matrix, density_curves
    )
...
def create_profile(
    ...
    ls, d, wl, c, le = load_agx_emulsion_data(
        stock=stock,
    ...
    if type == "negative" or type == "positive":
        ...
        profile.dir_couplers.high_exposure_shift = (
            0.0  # increase saturation and contrast with overexposure
        )

        profile.masking_couplers.cross_over_points = [585, 510, 200]
        profile.masking_couplers.transition_widths = [15, 15, 1]
        profile.masking_couplers.gaussian_model = [
            [[435, 20, 0.09], [560, 20, 0.09]],
            [[470, 20, 0.09]],
            [[520, 20, 0.09]],
        ]  # [wl, width, amount]
...
def adjust_log_exposure(
    profile, speed_point_density=0.2, stops_over_speed_point=3, midgray_transmittance=0.184
):
...
def apply_masking_couplers(profile, control_plot=True, effectiveness=1.0, model="erf"):
    ...
    if model == "erf":
        ...
        coupler_mask_spectral = (scipy.special.erf(wl_scaled) + 1 + effectiveness) / (
            2 + effectiveness
        )
...
def replace_fitted_density_curves(profile, control_plot=False):
    ...
    profile.data.density_curves = dc
    profile.data.density_curves_layers = compute_density_curves_layers(
        le, density_curves_fitting_parameters, type=type
    )
...
def process_negative_profile(
    ...
    profile = reconstruct_dye_density(
        profile, control_plot=True, model=dye_density_reconstruct_model
    )
...
def plot_profile(profile, unmixed=False, original=None):
    ...
    if original is not None:
        axs[0].plot(
            wavelengths,
            original.data.log_sensitivity[:, 0],
            alpha=0.5,
            color="tab:red",
            linestyle="--",
        )
        axs[0].plot(
            wavelengths,
            original.data.log_sensitivity[:, 1],
            alpha=0.5,
            color="tab:green",
            linestyle="--",
        )
        axs[0].plot(
            wavelengths,
            original.data.log_sensitivity[:, 2],
            alpha=0.5,
            color="tab:blue",
            linestyle="--",
        )
    ...
    if original is not None:
        ...
        axs[2].plot(
            wavelengths,
            original.data.dye_density[:, 1],
            alpha=0.5,
            color="tab:pink",
            linestyle="--",
        )
        axs[2].plot(
            wavelengths, original.data.dye_density[:, 2], alpha=0.5, color="gold", linestyle="--"
        )
