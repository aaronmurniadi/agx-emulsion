...
from agx_emulsion.process.physics.couplers import (
...
class Film(AgXEmulsion):
    ...
    def develop(
        ...
        density_cmy = interpolate_exposure_to_density(
            log_raw, self.density_curves, self.log_exposure, self.gamma_factor
        )
        ...
        if self.dir_couplers.active:
            ...
            dir_couplers_amount_rgb = self.dir_couplers.amount * np.array(
                self.dir_couplers.ratio_rgb
            )
            M = compute_dir_couplers_matrix(
                dir_couplers_amount_rgb, self.dir_couplers.diffusion_interlayer
            )
            # compute density curves before dir couplers
            density_curves_0 = compute_density_curves_before_dir_couplers(
                self.density_curves, self.log_exposure, M, self.dir_couplers.high_exposure_shift
            )
            ...
            log_raw_0 = compute_exposure_correction_dir_couplers(
                ...
                self.dir_couplers.high_exposure_shift,
            )
            # interpolated with corrected curves
            density_cmy = interpolate_exposure_to_density(
                log_raw_0, density_curves_0, self.log_exposure, self.gamma_factor
            )
        ...
        if self.grain.active and not bypass_grain:
            if not self.grain.sublayers_active:
                ...
                density_cmy_layers = interp_density_cmy_layers(
                    density_cmy, self.density_curves, self.density_curves_layers
                )
                density_max_layers = np.nanmax(self.density_curves_layers, axis=0)
                density_cmy = apply_grain_to_density_layers(
                    density_cmy_layers,
...
def interp_density_cmy_layers(density_cmy, density_curves, density_curves_layers):
    density_cmy_layers = np.zeros(
        (density_cmy.shape[0], density_cmy.shape[1], 3, 3)
    )  # x,y,layer,rgb
    for ch in np.arange(3):
        density_cmy_layers[:, :, :, ch] = fast_interp(
            np.repeat(density_cmy[:, :, ch, np.newaxis], 3, -1),
            density_curves[:, ch],
