...
from agx_emulsion.process.config import SPECTRAL_SHAPE, ENLARGER_STEPS
from agx_emulsion.process.utils.io import load_dichroic_filters, load_filter
...
def create_combined_dichroic_filter(
    wavelength,
    ...
    dichroics[1][wavelength <= 550] = -scipy.special.erf(
        (wavelength[wavelength <= 550] - edges[1]) / transitions[1]
    )
    dichroics[1][wavelength > 550] = scipy.special.erf(
        (wavelength[wavelength > 550] - edges[2]) / transitions[2]
    )
    ...
    total_filter = np.prod(
        ((1 - filtering_amount[:, None]) + dichroics * filtering_amount[:, None]), axis=0
    )
...
def filterset(
    ...
    total_filter = create_combined_dichroic_filter(
...
class DichroicFilters:
    ...
    def apply(self, illuminant, values=[0, 0, 0]):
        dimmed_filters = 1 - (1 - self.filters) * np.array(
            values
...
class GenericFilter:
    def __init__(
        ...
        if load_from_database:
            self.transmittance = load_filter(
                self.wavelengths, name, brand, type, percent_transmittance=data_in_percentage
            )
...
generic_lens_transmission = GenericFilter(
    name="canon_24_f28_is", type="lens_transmission", brand="canon", data_in_percentage=True
)
...
if __name__ == "__main__":
    ...
    plt.plot(
        ...
        schott_kg3_heat_filter.transmittance * generic_lens_transmission.transmittance,
    )
    plt.ylim((0, 1))
