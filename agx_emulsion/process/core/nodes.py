...
from agx_emulsion.process.physics.emulsion import (
    Film,
    compute_density_spectral,
    develop_simple,
    remove_viewing_glare_comp,
    compute_random_glare_amount,
)
from agx_emulsion.process.physics.diffusion import (
...
from agx_emulsion.process.utils.spectral_upsampling import (
    ...
    def process(self, image, context: PipelineContext):
        ...
        if params.camera.auto_exposure:
            ...
            autoexposure_ev = measure_autoexposure_ev(
                image, input_color_space, input_cctf, method=method
            )
    ...
    def process(self, image, context: PipelineContext):
        ...
        if preview_resize_factor * upscale_factor != 1.0:
            image = skimage.transform.rescale(
                image, preview_resize_factor * upscale_factor, channel_axis=2
            )
...
class ProfileChangesNode(Node):
    def process(self, image, context: PipelineContext):
        ...
        if params.print_paper.glare.compensation_removal_factor > 0:
            ...
            dc_out = remove_viewing_glare_comp(
                le,
                dc,
    ...
    def _rgb_to_film_raw(self, rgb, exposure_ev, params):
        ...
        if params.camera.filter_uv[0] > 0 or params.camera.filter_ir[0] > 0:
            band_pass_filter = compute_band_pass_filter(
                params.camera.filter_uv, params.camera.filter_ir
            )
...
class FilmDevelopmentNode(Node):
    ...
    def process(self, raw, context: PipelineContext):
        if context.data.get("return_early"):
            return raw
        ...
        density_cmy = film.develop(
            log_raw, pixel_size_um, use_fast_stats=params.settings.use_fast_stats
        )
...
class PrintExposureNode(Node):
    ...
    def process(self, density_cmy, context: PipelineContext):
        ...
        log_raw = self._spectral_lut_compute(
    ...
    def _film_density_cmy_to_print_log_raw(self, density_cmy, params):
        ...
        if not params.enlarger.just_preflash:
            ...
            raw_midgray_factor = self._compute_exposure_factor_midgray(
                sensitivity, print_illuminant, params
            )
    ...
    def _compute_print_illuminant(self, light_source, params):
        y_filter = (
            params.enlarger.y_filter_neutral * ENLARGER_STEPS + params.enlarger.y_filter_shift
        )
        m_filter = (
            params.enlarger.m_filter_neutral * ENLARGER_STEPS + params.enlarger.m_filter_shift
        )
    ...
    def _compute_preflash_illuminant(self, light_source, params):
        y_filter_preflash = (
            params.enlarger.y_filter_neutral * ENLARGER_STEPS
            + params.enlarger.preflash_y_filter_shift
        )
        m_filter_preflash = (
            params.enlarger.m_filter_neutral * ENLARGER_STEPS
            + params.enlarger.preflash_m_filter_shift
        )
        c_filter = params.enlarger.c_filter_neutral * ENLARGER_STEPS
        preflash_illuminant = color_enlarger(
            light_source, y_filter_preflash, m_filter_preflash, c_filter
        )
    ...
    def _compute_exposure_factor_midgray(self, sensitivity, print_illuminant, params):
        ...
        raw_midgray = rgb_to_raw_hanatos2025(
            rgb_midgray,
            sensitivity_neg,
            color_space="sRGB",
...
class ScanSpectralNode(Node):
    ...
    def _density_cmy_to_rgb(self, density_cmy, params):
        ...
        log_xyz = self._spectral_lut_compute(
            density_cmy_n,
            spectral_calculation,
            params,
            use_lut=params.settings.use_scanner_lut,
    ...
    def add_glare(self, xyz, illuminant_xyz, profile):
        if profile.glare.active and profile.glare.percent > 0:
            glare_amount = compute_random_glare_amount(
                profile.glare.percent, profile.glare.roughness, profile.glare.blur, xyz.shape[:2]
            )
    ...
    def _apply_cctf_encoding_and_clip(self, rgb, params):
        ...
        if params.io.output_cctf_encoding:
            rgb = colour.RGB_to_RGB(
                rgb, color_space, color_space, apply_cctf_decoding=False, apply_cctf_encoding=True
            )
