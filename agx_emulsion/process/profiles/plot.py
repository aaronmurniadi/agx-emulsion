...
def plot_grain_chart(profile=load_profile("kodak_portra_400_auc"), film_format_mm=35):
    # make test chart
    ...
    ax1.text(
        0.16,
        0.90,
        f"Particle area: {profile.grain.agx_particle_area_um2} $\mu$m$^2$",
    ...
    ax1.text(
        0.16,
        0.85,
        f"Particle area scale RGB: {profile.grain.agx_particle_scale}",
        transform=ax1.transAxes,
        ha="left",
        va="center",
    )
    ax1.text(
        0.16,
        0.80,
        f"Particle area scale sublayers: {profile.grain.agx_particle_scale_layers}",
        transform=ax1.transAxes,
        ha="left",
        va="center",
    )
    ax1.text(
        0.16,
        0.75,
        f"Uniformity RGB: {profile.grain.uniformity}",
        transform=ax1.transAxes,
        ha="left",
        va="center",
    )
    ax1.text(
        0.16,
        0.70,
        f"Density min RGB: {profile.grain.density_min}",
        transform=ax1.transAxes,
        ha="left",
        va="center",
    )
