...
from agx_emulsion.process.utils.fast_interp_lut import apply_lut_cubic_2d
from agx_emulsion.process.config import SPECTRAL_SHAPE, STANDARD_OBSERVER_CMFS
...
def compute_lut_spectra(
    lut_size=128, smooth_steps=1, lut_coeffs_filename="hanatos_irradiance_xy_coeffs_250304.lut"
):
...
def load_spectra_lut(filename="irradiance_xy_tc.npy"):
    data_path = importlib.resources.files("agx_emulsion.data.luts.spectral_upsampling").joinpath(
        filename
    )
...
def rgb_to_tc_b(
    rgb, color_space="ITU-R BT.2020", apply_cctf_decoding=False, reference_illuminant="D55"
):
...
def rgb_to_raw_mallett2019(
    RGB, sensitivity, color_space="sRGB", apply_cctf_decoding=True, reference_illuminant="D65"
):
    """
    ...
    lrgb = colour.RGB_to_RGB(
        RGB, color_space, "sRGB", apply_cctf_decoding=apply_cctf_decoding, apply_cctf_encoding=False
    )
...
def rgb_to_raw_hanatos2025(
    rgb, sensitivity, color_space, apply_cctf_decoding, reference_illuminant
):
    ...
    illuminant_midgray = rgb_to_spectrum(
        midgray_rgb,
        color_space=color_space,
    ...
    raw_midgray = np.einsum(
        "k,km->m", illuminant_midgray, sensitivity
    )  # use 0.184 as midgray reference
    return raw / raw_midgray[1]  # normalize with green channel
