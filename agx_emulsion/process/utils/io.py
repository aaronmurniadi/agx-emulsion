...
from agx_emulsion.process.config import LOG_EXPOSURE, SPECTRAL_SHAPE
...
def load_csv(datapkg, filename):
    ...
    """
    conv = (
        lambda x: float(x) if x != b"" else None
    )  # conversion function to take care of empty elements
...
def load_agx_emulsion_data(
    ...
    if color and type == "negative":
        maindatapkg = "agx_emulsion.data.film.negative"
    elif color and type == "positive":
        maindatapkg = "agx_emulsion.data.film.positive"
    elif color and type == "paper":
        maindatapkg = "agx_emulsion.data.paper"
...
def load_densitometer_data(type="status_A", spectral_shape=SPECTRAL_SHAPE):
    ...
    for i, channel in enumerate(channels):
        ...
        responsivities[:, i] = interpolate_to_common_axis(
            data, spectral_shape.wavelengths, extrapolate=False, method="linear"
        )
...
def load_dichroic_filters(wavelengths, brand="thorlabs"):
    ...
    for i, channel in enumerate(channels):
        ...
        with resource.open("r") as file:
            ...
            filters[:, i] = scipy.interpolate.Akima1DInterpolator(data[:, 0], data[:, 1] / 100)(
                wavelengths
            )
    return filters
...
def load_filter(
    ...
    with resource.open("r") as file:
        ...
        transmittance = scipy.interpolate.Akima1DInterpolator(data[:, 0], data[:, 1] / scale)(
            wavelengths
        )
