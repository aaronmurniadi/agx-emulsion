...
from agx_emulsion.process.utils.measure import measure_slopes_at_exposure
...
def density_mid_min_model(params, wl, cmy_model, model):
    ...
    if model[:7] == "filters":
        ...
        filters = np.stack((1 - (1 - hp_c) * (1 - lp_c), hp_m * lp_m, lp_y), axis=1)
    if model == "filters":
        cmy = dye * filters
    if model == "filters_neg":
        cmy = dye * filters
    ...
    if model[:10] == "filteramps":
        ...
        lp_c = low_pass_filter(
            wl, params["lp_c_wl"], params["lp_c_width"], params["lp_c_amp"]
        )  # test
        filters = np.stack((1 - (1 - hp_c) * (1 - lp_c), hp_m * lp_m, lp_y), axis=1)
    if model == "filteramps":
        cmy = dye * filters
    if model == "filteramps_gauss":
        hp_c_gauss = high_pass_gaussian(
            wl, params["hp_c_wl"], params["hp_c_width"], params["hp_c_amp"]
        )
        lp_c_gauss = low_pass_gaussian(
            wl, params["lp_c_wl"], params["lp_c_width"], params["lp_c_amp"]
        )
        hp_m_gauss = high_pass_gaussian(
            wl, params["hp_m_wl"], params["hp_m_width"], params["hp_m_amp"]
        )
        lp_m_gauss = low_pass_gaussian(
            wl, params["lp_m_wl"], params["lp_m_width"], params["lp_m_amp"]
        )
        lp_y_gauss = low_pass_gaussian(
            wl, params["lp_y_wl"], params["lp_y_width"], params["lp_y_amp"]
        )
        filters = np.stack((1 - (1 - hp_c) * (1 - lp_c), hp_m * lp_m, lp_y), axis=1)
        gauss = np.stack((hp_c_gauss + lp_c_gauss, hp_m_gauss + lp_m_gauss, lp_y_gauss), axis=1)
        # filters[:,0] += lp_c # test
...
def density_min_model(params, wl, cmy, cpl, channels_couplers_gaussians):
    ...
    for i in range(5):
        ...
        cpl_cmy[:, channels_couplers_gaussians[i]] += (
            cpl[:, i] / dcmy[channels_couplers_gaussians[i]] * dmax[channels_couplers_gaussians[i]]
        )
...
def slopes_of_concentrations(log_exposure, density_curves, dstm_cm):
    ...
    for i in range(3):
        for j in range(3):
            c[:, i] += (
                np.linalg.inv(dstm_cm)[i, j] * density_curves[:, j]
            )  # TODO fix order of indexes of crosstalk matrix according to formalism
...
def residual_simple(
    ...
    res = data - sim
    res = np.concatenate(
        (res, biases[1] * out_of_diagonal_crosstalk, biases[2] * diff_gammas)
    )  # add crosstalk matrix to the loss function
...
from agx_emulsion.process.profiles.io import load_profile, save_profile
...
def reconstruct_dye_density(
    ...
    if control_plot:
        ...
        if model == "dmid_dmin":
            dmin_sim, cpl, scat, fog, base, dmax = density_min_model(
                out.params, wl, cmy, filters, [0, 0, 1, 1, 2]
            )
            axs[2].plot(wl, np.ones_like(wl) * base, "tab:green", label="Base")
            axs[2].plot(wl, base + scat, "tab:blue", label="Scattering")
            axs[2].plot(
                wl,
                base + scat + np.sum(fog * cmy, axis=1) + cpl[:, 0],
                color="tab:cyan",
                label="Mask C",
            )
            axs[2].plot(
                wl,
                base + scat + np.sum(fog * cmy, axis=1) + cpl[:, 1],
                color="tab:pink",
                label="Mask M",
            )
            axs[2].plot(
                wl,
                base + scat + np.sum(fog * cmy, axis=1) + cpl[:, 2],
                color="gold",
                label="Mask Y",
            )
            axs[2].plot(
                wl, base + scat + np.sum(fog * cmy, axis=1), color="tab:orange", label="Fog"
            )
...
if __name__ == "__main__":
    ...
    negatives = ["kodak_portra_400", "fujifilm_pro_400h", "kodak_vision3_50d"]
    for neg in negatives:
        profile = load_profile(neg)
        profile = reconstruct_dye_density(profile, control_plot=True, print_params=True)
