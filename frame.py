...
def calculate_aspect_ratio_borders(
    width, height, target_aspect_width, target_aspect_height, base_border
):
    """
        ...
        try:
            if os.path.exists(font_path_attempt):
                ...
                if font_path_attempt.lower().endswith(".woff2"):
                    print(
                        f"⚠️  Warning: WOFF2 fonts are not supported by PIL/Pillow. Skipping '{font_path_attempt}'"
                    )
    ...
    try:
        ...
        with Image.open(image_path) as img:
            ...
            if watermark_text:
                ...
                text_y = (
                    border_size - font.getbbox(watermark_text)[3]
                ) // 2  # Center vertically in border
            ...
            if rotate_angle:
                if rotate_angle in [90, -90, 180, 270, -270]:
                    ...
                    print(
                        f"⚠️  Warning: Unsupported rotation angle {rotate_angle}. Supported: 90, -90, 180"
                    )
            ...
            if aspect_ratio:
                ...
                print(
                    f"  Additional borders: {additional_h_border}px (H), {additional_v_border}px (V)"
                )
...
Examples:
  ...
  python frame.py "image.png" --rotate -90 --watermark "Rotated" --percentage 3.0
    ...
    parser.add_argument(
        "--font", type=str, help="Path to custom font file in TTF/OTF format (WOFF2 not supported)"
    )
    parser.add_argument(
        "--percentage", type=float, default=4.0, help="Border percentage (default: 4.0)"
    )
    parser.add_argument(
        "--aspect", type=str, help='Target aspect ratio in format "x:y" (e.g., "16:9", "4:3")'
    )
    ...
    # Process the image
    output_path = add_border_and_watermark(
        args.image,
        args.watermark,
        args.output,
        args.font,
