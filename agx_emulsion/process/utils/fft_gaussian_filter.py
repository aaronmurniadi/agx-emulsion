    ...
    if image.ndim == 2:
        ...
        if parallel:
            ...
            if np.isscalar(sigma):
                ...
                if sigma.size != C:
                    raise ValueError(
                        "Length of sigma array must equal the number of channels in the image."
                    )
                for c in range(C):
                    filtered[..., c] = _fft_gaussian_filter_2d(
                        image[..., c], sigma[c], truncate, pad
                    )
        return filtered
    else:
...
if __name__ == "__main__":
    ...
    filtered_fft_3d_array = fft_gaussian_filter(
        image3d, sigma_array, truncate=7.0, pad=True, parallel=True
    )
    ...
    for c in range(image3d.shape[2]):
        filtered_scipy_3d_array[..., c] = gaussian_filter(
            image3d[..., c], sigma_array[c], mode="reflect"
        )
