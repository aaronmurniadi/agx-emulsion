...
def measure_log_exposure_midscale_neutral(profile, reference_channel=None):
    ...
    for i in range(3):
        if profile.info.type == "positive":
            log_exposure_midscale_neutral[i] = np.interp(
                -d_mid[i], -profile.data.density_curves[:, i], profile.data.log_exposure
            )
        else:
            log_exposure_midscale_neutral[i] = np.interp(
                d_mid[i], profile.data.density_curves[:, i], profile.data.log_exposure
...
def align_midscale_neutral_exposures(profile, reference_channel=None):
    ...
    profile.info.log_exposure_midscale_neutral = (
        np.ones(3) * log_exposure_midscale_neutral[1]
    ).tolist()
    return profile
...
def correct_negative_curves_with_gray_ramp(
    ...
    density_scale, shift_corr, stretch_corr = fit_corrections_from_grey_ramp(
        pl, data_trustability, stretch_curves
    )
    print("Density scale corr:", density_scale)
    print("Shift corr:", shift_corr)
    print("Stretch corr:", stretch_corr)
    profile_corrected = apply_scale_shift_stretch_density_curves(
        pl.negative, density_scale, shift_corr, stretch_corr
    )
...
def fit_corrections_from_grey_ramp(p0, data_trustability=1.0, stretch_curves=False):
    ...
    def residues(x):
        if stretch_curves:
            gray, reference = gray_ramp(
                p0, density_scale=x[0:3], shift_corr=[x[3], 0, x[4]], stretch_corr=x[5:8]
            )
        else:
            gray, reference = gray_ramp(p0, density_scale=x[0:3], shift_corr=[x[3], 0, x[4]])
    ...
    if stretch_curves:
        x0 = [1.0, 1.0, 1.0, 0.0, 0.0, 1.0, 1.0, 1.0]
    else:
...
def gray_ramp(p0, density_scale=[1, 1, 1], shift_corr=[0, 0, 0], stretch_corr=[1, 1, 1]):
    ...
    pl.settings.rgb_to_raw_method = "mallett2019"
    pl.negative = apply_scale_shift_stretch_density_curves(
        pl.negative, density_scale, shift_corr, stretch_corr
    )
    midgray_rgb = np.array([[[0.184, 0.184, 0.184]]])
...
def apply_scale_shift_stretch_density_curves(
    profile, density_scale=[1, 1, 1], log_exposure_shift=[0, 0, 0], log_exposure_strech=[1, 1, 1]
):
...
def heavy_lifting_density_curves(
    profile, density_max=False, log_exposure=False, gamma=False, gamma_correction_range_ev=2
):
    ...
    if gamma:
        gamma, le_ref = measure_slopes(
            profile, log_exposure_range=np.log10(2**gamma_correction_range_ev)
        )
...
def measure_slopes(profile, log_exposure_range=np.log10(2**2)):
    ...
    for i in range(3):
        ...
        density_1 = scipy.interpolate.CubicSpline(log_exposure[sel], density_curves[sel, i])(
            log_exposure_1[i]
        )
        density_0 = scipy.interpolate.CubicSpline(log_exposure[sel], density_curves[sel, i])(
            log_exposure_0[i]
        )
...
if __name__ == "__main__":
    from agx_emulsion.process.profiles.factory import (
        load_profile,
        plot_profile,
        adjust_log_exposure,
    )
    ...
    profile_hv = heavy_lifting_density_curves(
        profile, density_max=True, log_exposure=True, gamma=True
    )
    plot_profile(profile_hv)
    profile_corrected = correct_negative_curves_with_gray_ramp(
        profile, target_paper="kodak_portra_endura_uc", data_trustability=0.3, stretch_curves=False
    )
