...
def print_filter_test(
    ...
    y_filter_range = np.linspace(
        y_filter - y_filter_spread / 2, y_filter + y_filter_spread / 2, points
    )
    m_filter_range = np.linspace(
        m_filter - m_filter_spread / 2, m_filter + m_filter_spread / 2, points
    )
    ...
    for i, y_filter in enumerate(y_filter_range):
        for j, m_filter in enumerate(m_filter_range):
            ...
            filtered_illuminant = dichroic_filters.apply(
                print_illuminant, values=[y_filter, m_filter, c_filter]
            )
...
def grain_test(self, multilayer_grain=True, samples=256):
...
def test_exposure_bias(self, illuminant):
    ...
    ax.plot(
        np.log10(gray_scale), np.mean(gray_scale_image, axis=2)[:, 0], color="gray", linestyle="--"
    )
