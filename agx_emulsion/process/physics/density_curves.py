...
def distribution_model_norm_cdfs(
    loge,
    x,
    number_of_layers=3,
):
...
def density_curve_layers(loge, x, type="negative", number_of_layers=3):
    N = number_of_layers
    centers = x[0:N]
    amplitudes = x[N : 2 * N]
    sigmas = x[2 * N : 3 * N]
...
def guess_start_and_bounds_norm_cdfs(loge, data, type):
    ...
    if np.logical_or(type == "positive", type == "paper"):
        ...
        x_lb = [
            ...
            0.2,
            0.2,
            0.2,  # 0.35 in order to have a larger sensitive layer
            0.05,
            0.05,
            0.05,
        ]
        x_ub = [
            np.max(loge),
            np.max(loge),
            np.max(loge),  # Fixed: use max(loge) not max(data) for centers
            density_max_layer,
            density_max_layer,
            density_max_layer,
            2,
...
def density_curve_model_log_line(
    ...
    morph_shoulder = (
        ...
        * (
            1
            + curvature_shoulder_max
            * sigmoid(-curvature_shoulder_slope * (logE - H0 - D_range / gamma))
        )
    )
    morph_toe = (
        gamma
    ...
    rise = gamma / morph_toe * np.log10(1 + 10 ** (morph_toe * (logE - H0)))
    stop = (
        (gamma)
        / morph_shoulder
        * np.log10(1 + 10 ** (morph_shoulder * (logE - D_range / np.abs(gamma) - H0)))
    )
    if type == "positive":
        D = D_min - rise + stop
    else:
...
def guess_start_and_bounds_log_line(loge, data, type):
    ...
    x_ub = [
        ...
        8,
        4 * s,  # toe shape, slope-max
        8,
        4 * s,  # shoulder shape, slope-max
    ]
    return x0, (x_lb, x_ub)
...
def fit_density_curve(loge, data, type="negative", model="norm_cdfs"):
    if model == "norm_cdfs":
        model_function = density_curve_model_norm_cdfs
...
def fit_density_curves(
    density, model="norm_cdfs", type="negative", stock="film_stock", plotting=False
):
    ...
    for i in np.arange(3):
        fitted_parameters[i] = fit_density_curve(
            log_exposure, density.raw_data[:, i], type, model=model
        )
    ...
    if plotting:
        ...
        for i in np.arange(3):
            ...
            ax.plot(
                log_exposure,
                model_function(log_exposure, fitted_parameters[i], type),
                color=colors[i],
            )
            if model == "norm_cdfs":
                ax.plot(
                    log_exposure,
                    distribution_model_norm_cdfs(log_exposure, fitted_parameters[i]),
                    label="_nolegend_",
                    color=colors[i],
                    linewidth=1,
                    linestyle="dashed",
